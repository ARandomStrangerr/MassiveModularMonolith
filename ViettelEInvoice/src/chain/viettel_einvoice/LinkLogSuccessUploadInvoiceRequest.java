...
public class LinkLogSuccessUploadInvoiceRequest extends Link {
	...
	public boolean execute() {
		...
		monitorObj.addProperty("notification", String.format("Đơn vị với mã số thuế %s thành công tải lên %d hoá đơn", chain.getProcessObject().get("body").getAsJsonObject().get("username").getAsString(), chain.getProcessObject().get("body").getAsJsonObject().get("totalNumber").getAsInt()));
